...
{
	string day = to_string(dt.day);
	if (day.size() == 1)
		day = "0" + day;
	string month = to_string(dt.month);
	if (month.size() == 1)
		month = "0" + month;
	string year = to_string(1980 + dt.year);
	string hour = to_string(dt.hour);
	if (hour.size() == 1)
		hour = "0" + hour;
	string minute = to_string(dt.minute);
	if (minute.size() == 1)
		minute = "0" + minute;
	string second = to_string(dt.second);
	if (second.size() == 1)
		second = "0" + second;
